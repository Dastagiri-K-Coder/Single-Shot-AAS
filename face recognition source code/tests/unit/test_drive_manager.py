...
def test_format_sheet_name_basic():
    ...
    result = format_sheet_name(1, "CSE", 2024, 2027, "A")
    assert result == "Single Shot AAS -1 CSE Y(2024-2027) S(A)"


def test_format_sheet_name_serial_increment():
    from drive_manager import format_sheet_name
    assert format_sheet_name(2, "MECH", 2023, 2026, "B") == "Single Shot AAS -2 MECH Y(2023-2026) S(B)"


def test_format_sheet_name_numeric_section():
    from drive_manager import format_sheet_name
    assert format_sheet_name(3, "ECE", 2025, 2028, "1") == "Single Shot AAS -3 ECE Y(2025-2028) S(1)"


@patch("drive_manager._build_drive")
...
def test_check_drive_setup_exists_true(mock_build_drive):
    ...
    assert check_drive_setup_exists(MagicMock()) is True
...
def test_check_drive_setup_exists_false(mock_build_drive):
    ...
    mock_build_drive.return_value = mock_service
    mock_service.files().list().execute.return_value = {"files": []}
    assert check_drive_setup_exists(MagicMock()) is False
